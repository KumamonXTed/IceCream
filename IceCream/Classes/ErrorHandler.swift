    ...
    func resultType(with error: Error?) -> CKOperationResultType {
        ...
        case .serviceUnavailable,
            ...
            if let retry = userInfo[CKErrorRetryAfterKey] as? Double {
                logInfo("ErrorHandler - \(message). Should retry in \(retry) seconds.")
        ...
        case .networkUnavailable,
             .networkFailure:
            logInfo("ErrorHandler.recoverableError: \(message)")
        ...
        case .partialFailure:
            ...
            if let dictionary = e.userInfo[CKPartialErrorsByItemIDKey] as? NSDictionary {
                logInfo("ErrorHandler.partialFailure for \(dictionary.count) items; CKPartialErrorsByItemIDKey: \(dictionary)")
            }
            return .recoverableError(reason: .partialFailure, message: message)
            
        // SHOULD CHUNK IT UP
        case .limitExceeded:
            logInfo("ErrorHandler.Chunk: \(message)")
            return .chunk
        ...
        case .alreadyShared,
             ...
             .referenceViolation,
             .tooManyParticipants:
            logInfo("ErrorHandler.Fail: \(message)")
